...
class Auth extends Controller
{
    ...
    public function login($data): void
    {
        ...
        echo $this->ajaxRespose("redirect", ["url" => $this->router->route("app.home")]);
    ...
    public function register($data): void 
    {
        ...
        $_SESSION["user"] = $user->id;
        echo $this->ajaxRespose("redirect", [
    ...
    public function forget($data): void
    {
        ...
        flash("seccess", "Enviamos um link de recuperação para seu e-mail.");
    ...
    public function reset($data): void
    {
        if(empty($_SESSION["forget"]) || !$user = (new User())->findById($_SESSION["forget"])){
            flash("error", "Não foi possivl recuperar, tente novamente");
            echo $this->ajaxRespose("redirect", [
                "url" => $this->router->route("web.forget")
            ]);
            return;
        ...
        if($data["password"] != $data["password_re"]){
            echo $this->ajaxRespose("message", [
                "type" => "error",
                "message" => "Senhas informadas não CONFERE."
            ]);
            return;
        }
    ...
    public function facebook(): void
    {
        ...
        $user_by_id = (new User())->find("facebook_id = :d", "id={$facebook_user->getId()}")->fetch();
    ...
    public function google(): void
    {
        ...
        if(!$error && !$code){
            $auth_url = $google->getAuthorizationUrl(["scope" => "email"]);
            header("Location: " {$auth_url});
            return;
        }
        ...
        $user_by_id = (new User())->find("google_id = :d", "id={$google_user->getId()}")->fetch();
